...
export default class JupyterServer {
  ...
  async _getKernel(key, serverSettings_) {
    ...
    const {
      serverSettings,
      kernelModel
    } = await this._getKernelModel(baseUrl, token, kernelId)
  ...
  async startServer({
    ...
    if (this.cached_servers[config_str]) {
      const {
        url,
        token
      } = this.cached_servers[config_str]
      server_url = url
      server_token = token
      try {
    ...
    if (!this.registered_file_managers[server_url]) {
      ...
      this.registered_file_managers[url] = {
        url: url,
        contents: contents
      };
    }
  ...
  installRequirements(kernel, reqs, conda_available) {
    return new Promise(async (resolve, reject) => {
      ...
      let execution = kernel.requestExecute({
        code: commands.join("\n")
      })
